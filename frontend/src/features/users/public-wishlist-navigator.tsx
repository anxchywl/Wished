...
function PublicWishRow({ wish, shareToken, onOpen, onPrefetch }: PublicWishRowProps) {
  ...
  const isGroupGifted = !isCompleted && hasActiveGroupGift;
  const isLockedBooked = !isCompleted && !isGroupGifted && isBooked;
  return (
    <button
      type="button"
      className="public-row-button"
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className={`min-w-0 text-left ${isLockedBooked || isGroupGifted ? "opacity-50" : ""}`}>
...
function PublicWishView({
  ...
  const liveGroupGift = useGroupGiftQuery(wishId, shareToken);
