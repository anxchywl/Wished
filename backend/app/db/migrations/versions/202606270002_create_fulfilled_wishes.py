...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "group_gift_id",
            UUID(as_uuid=True),
            sa.ForeignKey("group_gifts.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column(
            "organizer_user_id",
        ...
        sa.Column(
            "fulfilled_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False
        ),
        sa.Column(
        ...
        sa.CheckConstraint(
            "source IN ('booking', 'group_gift')", name="ck_fulfilled_wishes_source"
        ),
    )
    op.create_index(
        op.f("ix_fulfilled_wishes_wish_id"), "fulfilled_wishes", ["wish_id"], unique=False
    )
    op.create_index(
        op.f("ix_fulfilled_wishes_participant_user_id"),
        "fulfilled_wishes",
        ["participant_user_id"],
        unique=False,
    )
