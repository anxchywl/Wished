...
async def check_wishlist_create_limit(
    redis: Redis, user_id: UUID, per_hour: int, per_day: int
) -> None:
    uid = str(user_id)
    logger.debug("wishlist create rate check user=%s", uid)
    await check_dual_rate_limit(
        redis, "rate:wishlist:create", uid, per_hour, per_day, "wishlist creation "
    )
